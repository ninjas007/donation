                ...
                <form action="{{ route('admin.campaign.update', $campaign->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $campaign->title) }}" placeholder="Judul Campaign"
                        ...
                        <div>
                            ...
                            <select name="category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" class="py-1" @if($campaign->category_id == $category->id) selected @endif>{{ $category->name }}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                        ...
                        <div>
                            ...
                            <input type="date" value="{{ old('max_date', $campaign->max_date) }}" name="max_date"
                                class="form-input w-full mt-2-rounded-md bg-gray-200 focus:bg-white p-3">
                            @error('max_date')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
