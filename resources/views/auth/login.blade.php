...
<div class="flex justify-center items-center h-screen bg-gray-300 px-6">
    <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
        <div class="flex justify-center items-center">
            <span class="text-gray-700 font-semibold text-2x1">LOGIN</span>
        </div>
        @if (session('status'))
        ...
        <form action="{{ route('login') }}" method="POST" class="mt-4">
            @csrf
            <label class="block">
                <span class="classtext-gray-700 text-sm">Email</span>
                <input type="email" name="email" value="{{ old('email') }}"
                    class="form-input mt-1 block w-full rounded-md focus:outline-none" placeholder="Alamat Email">
                @error('email')
                <div class="inline-flex max-w-sm w-full bg-red-200 shadow-sm rounded-sm overflow-hidden mt-2">
                    <div class="px-4 py-2">
            ...
            <label class="block mt-3">
                ...
                <input type="password" name="password" class="form-input mt- block w-full rounded-md"
            ...
            <div class="flex justify-between items-center mt-4">
                <div>
                    <label class="inline-flex items-center">
                        <input type="checkbox" class="form-checkbox tetx-indigo-600">
                        <span class="mx-2 text-gray-600 text-sm">Ingatkan Saya</span>
                    </label>
                </div>
                <a href="/forgot-password" class="block text-sm fontme text-indigo-700 hover:underline">Lupa
                    Password?</a>
            </div>
