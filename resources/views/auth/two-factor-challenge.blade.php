...
<div class="flex justify-center items-center h-screen bg-gray-300 px-6">
    <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
        <div class="flex justify-center items-center">
            <span class="text-gray-700 text-2xl font-semibold">TWO FACTOR CHALLENGE</span>
        </div>
        @if (session('status'))
        ...
        @endif
        <form action="{{ url('/two-factor-challenge') }}" method="post" class="mt-4">
            @csrf
            <label class="block">
                <span class="text-gray-700 text-sm">Code</span>
                <input type="text" name="code" value="{{ old('email') }}" placeholder="Code" class="form-input mt-1 block w-full rounded-md focus:border-indigo-600">
                @error('email')
            ...
            <p class="text-gray-600 mt-3"> <i>Atau Anda dapat memasukkan salah satu recovery code</i> </p>
            ...
            <label class="block mt-3">
                ...
                <div class="inline-flex max-w-sm w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                    <div class="px-4 py-2">
                        <p class="text0gray-600 text-sm">{{ $message }}</p>
                    </div>
                </div>                    
                @enderror
            </label>
